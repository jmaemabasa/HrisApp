...
namespace HrisApp.Server.Controllers.ImageC
{
    ...
    [ApiController]
    public class AssLicenseImageController : ControllerBase
    {
        private readonly DataContext _context;
        private readonly IWebHostEnvironment _evs;
        private readonly ILogger<ImageController> _logger;

        public AssLicenseImageController(DataContext context, IWebHostEnvironment evs, ILogger<ImageController> logger)
        {
            _context = context;
            _evs = evs;
            _logger = logger;
            ...
            try
            {
                ...
                if (httpRequest.Form.Files.Count > 0)
                {
                    foreach (var file in httpRequest.Form.Files)
                    {
                        ...
                        //update
                        var db = await _context.AssLicenseImageT.Where(a => a.JM_Code == jmcode && a.SubCategoryId == subcat).FirstOrDefaultAsync();
                        if (db != null)
                        {
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<AssLicenseImageT>> GetSingleImage(int id)
        {
            var user = await _context.AssLicenseImageT
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> UpdateDBImage(AssLicenseImageT img, int id)
        {
            var dbimg = await _context.AssLicenseImageT.FirstOrDefaultAsync(d => d.Id == img.Id);
        ...
        //GEEET
        [HttpGet]
        public async Task<ActionResult<List<AssLicenseImageT>>> GetObjList()
        {
        ...
        {
            var filteredDiv = await _context.AssLicenseImageT.Where(x => x.JM_Code == jmcode).OrderByDescending(e => e.Img_Date).ToListAsync();
            return Ok(filteredDiv);
        }

        [HttpDelete("DeleteAssetImg")]
        public async Task<ActionResult<List<AssLicenseImageT>>> DeleteAssetImg([FromQuery] string filename, [FromQuery] string jmcode)
        {
            var dbcol = await _context.AssLicenseImageT
            ...
            _context.AssLicenseImageT.Remove(dbcol);
