...
#nullable disable
    ...
    public partial class AssLicenseMultiImage : ComponentBase
    {
        [Parameter] public int ObjAssetId { get; set; }
        [Parameter] public string ObjAssetCode { get; set; } = string.Empty;
        ...
        private List<AssLicenseImageT> listOfImages = new();
        ...
        {
            await Task.Delay(0);
            listOfImages.Add(new AssLicenseImageT());
        }
        ...
        private async Task OnPDFSaving()
        {
            ...
            if (confirmResult.IsConfirmed)
            {
                ...
                {
                    await AssLicenseImgSvc.AttachFilePanel(formdata, Catid, SubCatid, JMCode, _remarks);
                }
                OnFileAdded?.Invoke();
                visible = false;
            }
