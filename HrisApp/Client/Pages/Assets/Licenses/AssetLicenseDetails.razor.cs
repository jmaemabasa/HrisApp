...
#nullable disable
    ...
    public partial class AssetLicenseDetails : ComponentBase
    {
        ...
        private List<AssLicenseImageT> assetImgList = new();
        private List<AssetLicenseRemarksT> REMARKS = new();
        ...
        private string ObjImageData { get; set; } = string.Format("images/asset-holder.jpg");
        ...
        protected override async Task OnParametersSetAsync()
        {
            try
            {
                ...
                REMARKS = await LicenseRemarksSvc.GetObjList(obj.AssetCode);
                ...
                {
                    ObjImageData = string.Format("images/asset-holder.jpg");
                }

                await GenerateQR(obj.Id);

                await AssLicenseImgSvc.GetAllImagesPerAss(obj.JMCode);
                assetImgList = AssLicenseImgSvc.AssLicenseImageTs;
        ...
        {
            var listApi = await LicenseRemarksSvc.GetObjList(posCode);
            List<string> arrApi = new();

            var validRemark = REMARKS
               .Where(obj => !string.IsNullOrEmpty(obj.LicenseAssetCode))
               .ToList();
            ...
            if (listOfValidRemarks.Count == 0)
            {
                ...
                {
                    await LicenseRemarksSvc.DeleteAllObj(item.VerifyId);
                }
                return;
            }
            ...
            foreach (var pays in listApi)
            {
                ...
                if (P == 0)
                {
                    await LicenseRemarksSvc.DeleteAllObj(pays.VerifyId);
                }
            }

            foreach (var item in listOfValidRemarks)
            {
                item.LicenseAssetCode = posCode;
        ...
        public void AddNewRemark(string code, string newSkill)
        {
            ...
            if (!string.IsNullOrEmpty(newRemark))
                REMARKS.Add(new AssetLicenseRemarksT { LicenseAssetCode = code, Remark = newSkill, VerifyId = verifyCode });
            newRemark = "";
        ...
        private async Task DeleteAssetImg(string filename, string assetcode)
        {
            ...
            if (confirmResult.IsConfirmed)
            {
                await AssLicenseImgSvc.DeleteAssetImg(filename, assetcode);
        ...
        {
            NavigationManager.NavigateTo("/asset-licenses");
        }
        ...
        public async Task ReloadObjects()
        {
            ...
            REMARKS = await LicenseRemarksSvc.GetObjList(obj.AssetCode);
            obj = await AssLicenseSvc.GetSingleObj((int)Id);
        ...
        private async Task LoadAccessImg(string jmcode)
        {
            ...
            {
                var base642 = Convert.ToBase64String(imagemodel);
                ObjImageData = string.Format("data:image/png;base64,{0}", base642);
            }
        }
        ...
        private async Task GenerateQR(int id)
        {
            ...
            QRCodeData qrCodeData = qrGenerator.CreateQrCode($"http://sonicsales.net:1113/asset-license/details/{id}", QRCodeGenerator.ECCLevel.H);
        ...
        private async Task PrintQR()
        {
            try
            {
                ...
                string url = await AssLicenseSvc.QRGenerate(obj.AssetCode);
